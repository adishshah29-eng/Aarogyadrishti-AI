...
* **Deployed-model ablation** (5-fold CV, isolated vs. chained checkup-safe): Heart Disease **~neutral** (~0.00% acc, +0.001 AUC); Hypertension **~neutral** (+0.10% acc, +0.000 AUC). On both deployed downstream models the upstream signal is largely already captured by the vitals.
